  ...
  return (
    <section id="home" className="relative min-h-screen w-full flex items-center justify-center bg-navy-dark overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 py-32 flex flex-col justify-center relative z-10 w-full min-h-screen">
        <div className="max-w-4xl flex flex-col space-y-8 text-left">
          {/* Slogan */}
          <motion.div
            initial={{ opacity: 0, y: 15 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8 }}
            className="inline-flex items-center space-x-3 text-gold-warm tracking-[0.3em] text-xs sm:text-sm uppercase font-bold"
          >
            <span className="flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-gold-warm animate-pulse" />
              {banner?.slogan || "CURATORS OF FINE ARTISTRY"}
            </span>
          </motion.div>
          ...
          {/* Main Title & Description inside premium glass panel */}
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 1, delay: 0.2 }}
            className="space-y-6 glass-premium p-6 sm:p-10 rounded-lg border-glow-hover"
          >
            <h1 className="text-4xl sm:text-6xl md:text-7.5xl font-secondary font-black text-white leading-[1.1] tracking-tight">
              {banner?.title ? (
                <span className="whitespace-pre-line">{formatHeading(banner.title)}</span>
              ) : (
                <>
                  Sculpting <span className="text-gradient-gold italic font-secondary font-light">Light</span> & <br />
                  Capturing Emotion
                </>
              )}
            </h1>
            ...
            {/* Description */}
            <p className="text-white/80 text-base md:text-lg max-w-2xl font-light leading-relaxed font-primary">
          ...
          >
            <a
              ...
              className="relative inline-flex items-center justify-center space-x-3 bg-gold-warm text-navy-dark px-8 py-4 text-xs font-bold uppercase tracking-widest hover:bg-white hover:text-navy-dark transition-all duration-500 rounded-xs shadow-lg hover:shadow-xl group"
            >
            ...
            <a
              ...
              href="#gallery"
              className="inline-flex items-center justify-center px-8 py-4 text-xs font-bold uppercase tracking-widest text-white border border-white/20 hover:border-gold-warm hover:text-gold-warm transition-all duration-300 rounded-xs bg-transparent"
            >
              View Gallery
            </a>
          </motion.div>
      ...
      >
        <div className="flex space-x-12">
          <div>
            ...
            <span className="font-semibold text-white/80 font-secondary">SkyMotion Studio</span>
          </div>
          <div>
