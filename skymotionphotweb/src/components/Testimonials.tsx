...
export default function Testimonials() {
  ...
  return (
    <section id="testimonials" className="py-24 md:py-32 bg-navy-dark relative overflow-hidden border-t border-b border-white/5">
      {/* Absolute Decorative Golden Quote Mark Backdrop */}
      <div className="absolute right-12 top-10 text-gold-warm/3 select-none pointer-events-none hidden lg:block">
      ...
      <div className="max-w-4xl mx-auto px-6 md:px-12 text-center relative z-10">
        ...
        <div className="space-y-4 mb-16">
          ...
          <div className="h-[2px] w-12 bg-gold-warm mx-auto" />
        </div>
        ...
        <div className="relative min-h-[380px] md:min-h-[300px] flex items-center justify-center">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="glass-premium p-8 md:p-12 shadow-xl hover:shadow-2xl hover:border-gold-warm/40 group duration-500 rounded-sm relative w-full"
            >
              ...
              <blockquote className="font-secondary text-lg md:text-xl text-white/95 leading-relaxed font-semibold italic mb-8">
                “{current.quote}”
              </blockquote>
              ...
              <div className="flex flex-col md:flex-row items-center justify-center space-y-4 md:space-y-0 md:space-x-4">
                ...
                <div className="text-center md:text-left">
                  <p className="font-secondary text-md font-bold text-white tracking-wide">
                    {current.clientName}
                  </p>
                  <p className="font-primary text-[10px] tracking-widest text-gold-warm uppercase font-bold mt-0.5">
                    {current.role} <span className="text-white/30 mx-1">•</span> {current.companyName}
                  </p>
                </div>
              </div>
            </motion.div>
        ...
        <div className="flex items-center justify-between mt-8 max-w-xs mx-auto">
          ...
          <button
            ...
            onClick={handlePrev}
            className="p-3 bg-white/5 border border-white/10 text-white hover:border-gold-warm hover:text-gold-warm rounded-full duration-300 shadow-xs cursor-pointer focus:outline-hidden"
            ...
            {items.map((item, index) => (
              <button
                ...
                className={`h-2 rounded-full transition-all duration-300 ${
                  index === currentIndex ? "w-6 bg-gold-warm" : "w-2 bg-white/10"
                }`}
