...
export default function Gallery({ isHomepage = false }: GalleryProps) {
  ...
  return (
    <section id="gallery" className="py-24 md:py-32 bg-navy-dark">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
          ...
          <div className="flex flex-wrap gap-2 md:gap-3 max-w-2xl">
            {categoriesList.map((category) => (
              <button
                ...
                className={`px-4 py-2 text-[10px] tracking-widest uppercase font-bold border transition-all duration-300 rounded-xs ${
                  selectedCategory === category
                    ? "bg-gold-warm border-gold-warm text-navy-dark shadow-md shadow-gold-warm/20"
                    : "bg-white/5 border-white/10 text-white/75 hover:border-gold-warm hover:text-gold-warm hover:bg-white/10"
                }`}
              >
                {category}
              </button>
          ...
          <AnimatePresence mode="popLayout">
            {displayItems.map((item) => (
              <motion.div
                ...
                className="break-inside-avoid mb-6 group relative cursor-pointer overflow-hidden shadow-lg border border-white/5 bg-white/5 rounded-xs transition-shadow hover:shadow-xl"
                onClick={() => openLightbox(item.id)}
              >
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all duration-500 z-10 flex items-center justify-center">
                  <span className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 transform translate-y-3 group-hover:translate-y-0 z-20 glass-premium text-white font-primary text-[10px] tracking-widest uppercase font-semibold px-4 py-2 flex items-center space-x-2 border border-white/10">
        ...
        {displayItems.length === 0 && (
          <div className="text-center py-20 border border-dashed border-white/10 rounded-xs bg-white/2">
            <p className="font-secondary text-lg text-white/50 italic">No items found under this luxury subset.</p>
          </div>
        )}
        ...
        {isHomepage && displayItems.length > 0 && (
          <div className="mt-16 text-center">
            <Link
              ...
              to="/gallery"
              className="inline-flex items-center space-x-2 px-8 py-4 border border-white/10 text-white hover:border-gold-warm hover:bg-white/5 font-primary text-xs tracking-widest uppercase font-bold transition-all duration-300 rounded-xs"
            >
      ...
      <AnimatePresence>
        {activeImageIndex !== null && activeImage && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-black/95 z-55 flex flex-col justify-between p-6 md:p-10 backdrop-blur-md"
          ...
          >
            ...
            <div className="flex items-center justify-between pointer-events-auto h-12 w-full max-w-7xl mx-auto z-10">
              ...
              <button
                ...
                onClick={closeLightbox}
                className="p-3 bg-white/5 hover:bg-white/15 transition-all rounded-full text-white cursor-pointer border border-white/5"
            ...
            <div className="relative flex-1 flex items-center justify-center max-w-5xl mx-auto w-full group/viewer">
              ...
              <button
                ...
                onClick={showPrev}
                className="absolute left-4 z-20 p-4 bg-white/5 hover:bg-white/15 text-white transition-all rounded-full opacity-60 hover:opacity-100 cursor-pointer border border-white/5"
              ...
              <motion.div
                ...
                className="relative max-h-[62vh] md:max-h-[70vh] max-w-full drop-shadow-2xl border-4 border-white/10 rounded-md overflow-hidden bg-black"
              ...
              <button
                ...
                onClick={showNext}
                className="absolute right-4 z-20 p-4 bg-white/5 hover:bg-white/15 text-white transition-all rounded-full opacity-60 hover:opacity-100 cursor-pointer border border-white/5"
            ...
            <div
              className="w-full max-w-3xl mx-auto glass-premium p-5 md:p-6 pointer-events-auto border-glow-hover"
              onClick={(e) => e.stopPropagation()}
            >
